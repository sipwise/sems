...
int AmSipDialog::onTxRequest(AmSipRequest& req, int& flags)
{
  ...
  if (offeranswer_enabled && oa.onRequestOut(req, (flags & SIP_FLAGS_NO_SDP_GENERATION) != 0))
    return -1;
...
int AmSipDialog::onTxReply(const AmSipRequest& req, AmSipReply& reply, int& flags)
{
  if (offeranswer_enabled) {
    AmMimeBody sdp_body;
    bool no_sdp_generation = req.body.empty() || (flags & SIP_FLAGS_NO_SDP_GENERATION);
    if(oa.onReplyOut(reply, flags, sdp_body, no_sdp_generation) < 0)
      return -1;
