...
struct AmConfig
{
  ...
  /** Who sends RTCP reports to the remote side per stream, when
   *  RtcpSendInterval > 0. Exactly one source is always active:
   *    auto     - own streams: SEMS sends reports;
   *               relayed streams: relay incoming RTCP (default);
   *    generate - SEMS sends reports for all streams;
   *    passthru - SEMS never sends reports, only relays incoming RTCP.
   *  Incoming RTCP is parsed for stream statistics in all modes. */
  enum RtcpReportMode { RtcpAutoMode = 0, RtcpGenerateMode, RtcpPassthruMode };
  /** Interval in seconds between RTCP reports generated by SEMS;
   *  0 (default) disables own report generation (incoming RTCP is still
   *  relayed and parsed for statistics). Recommended value per RFC 3550: 5. */
  static unsigned int RtcpSendInterval;
  static RtcpReportMode RtcpMode;
